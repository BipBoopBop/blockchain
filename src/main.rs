...
fn mine_block(id: u64, timestamp: i64, previous_hash: &str, data: &str) -> (u64,String) {
    ...
    loop{
        if nonce % 100000 == 0 { info!("nonce: {}", nonce); }
...
impl App {
    ...
    fn is_block_valid(&self, block: &Block, previous_block: &Block) -> bool {
        if block.previous_hash != previous_block.hash {
            warn!("block id: {} has wrong previous hash", block.id);
            return false;
        } 
        else if !hash_to_binary_representation(&hex::decode(&block.hash).expect("cannot decode from hex")).starts_with(DIFFICULTY_PREFIX){
    ...
    fn try_add_block(&mut self, block: Block) {
        let latest_block = self.blocks.last().expect("no previous block");
    ...
    fn choose_chain(&mut self, local: Vec<Block>, remote: Vec<Block>) -> Vec<Block> {
        ...
        if is_local_valid && is_remote_valid {
            if local.len() >= remote.len() { local }
            else { remote }
        }
        else if is_remote_valid { remote }
        else if is_local_valid { local }
        else { panic!("local and remote are invalid") }
    }
}
...
async fn main() {
    ...
    let auth_keys = Keypair::<X25519Spec>::new().into_authentic(&p2p::KEYS).expect("cannot create auth keys");
    ...
    Swarm::listen_on(
        &mut swarm,
        "/ip4/0.0.0.0/tcp/0".parse().expect("cannot get a local socket"),
    ).expect("swarm cannot be started");

    spawn(async move {
        sleep(Duration::from_secs(1)).await;
        info!("sending init event");
        init_sender.send(true).expect("cannot send init event");
    });

    loop{
        let evt = {
            select! {
                line = stdin.next_line() => Some(p2p::EventType::Input(line.expect("cannot get line").expect("cannot read line from stdin"))),
        ...
        if let Some(event) = evt {
            match event {
                p2p::EventType::Init => {
                    ...
                    if !peers.is_empty() {
                        let req = p2p::LocalChainRequest {
                            from_peer_id: peers.iter().last().expect("no peers").to_string(),
                        };
                ...
                p2p::EventType::Input(line) => match line.as_str() {
                    "id" => info!("Peer Id: {}", p2p::PEER_ID.clone()),
                    "ls p" => p2p::handle_print_peers(&swarm),
                    "sync" => p2p::handle_sync_chain(&mut swarm),
                    "sync all" => p2p::handle_sync_all(&mut swarm),
